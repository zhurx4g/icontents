...
function treeNode(id, pid, name, url, target, open,enable) {
	//public
	var obj = new Object();
	obj.id = id;
	obj.pid = pid;
...
function dzTree(treeName) {
	...
	this.addNodeExt = function(id, pid, name, url, target, open,enable) {
		var theNode = new treeNode(id, pid, name, url, target, open,enable);
		this.pushNodes.push(id);
		if(!this.nodes[pid]) {
			this.nodes[pid] = new Array();
		}
	...
	this.getName = function(theNode) {
		if(theNode.url) {
			if(theNode.enable==false){
				return '<a style="color:#a0a0a0;" onclick="return unauth();" href="'+theNode.url+'" target="' + theNode.target + '" > '+theNode.name+'</a>';				
			}else{
				return '<a href="'+theNode.url+'" target="' + theNode.target + '" > '+theNode.name+'</a>';
			}
		} else {
			if(theNode.enable==false){
				return '<a style="color:#a0a0a0;" onclick="return unauth();" href="javascript:' + treeName + '.switchDisplay(\'' + theNode.id + '\')">'+theNode.name+"</a>";				
			}else{
				return '<a href="javascript:' + treeName + '.switchDisplay(\'' + theNode.id + '\')">'+theNode.name+"</a>";
			}
		}
	}
